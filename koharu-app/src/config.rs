...
/// Remote backend config for offloading ML inference to a Colab GPU (or any
/// other machine running `koharu --headless`). When enabled, the local server
/// proxies all `/api/v1/*` requests to `url`.
...
pub fn load() -> Result<AppConfig> {
    ...
    // Allow overriding the remote backend URL via environment variable.
    // This is the easiest way to enable remote mode without editing config.toml.
    if let Ok(url) = std::env::var("KOHARU_REMOTE_URL") {
        if !url.is_empty() {
